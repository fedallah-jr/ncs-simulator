...
def main() -> None:
    ...
    if resuming:
        ...
        run_dir = prepare_run_directory("qplex", args.config, args.output_root)
    ...
    if (state_dim - 1) % n_agents != 0:
        raise ValueError("global_state must be per-agent blocks plus a single global channel state")
    unit_dim = (state_dim - 1) // n_agents
    ...
    with rewards_csv_path.open(csv_mode, newline="", encoding="utf-8") as train_f, \
        ...
        while global_step < args.total_timesteps:
            ...
            next_global_state_raw = np.asarray(step.infos.get("global_state"), dtype=np.float32)

            rewards = step.rewards_arr
