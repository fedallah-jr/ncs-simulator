"""Recurrent QMIX (NDQ-style GRU agent, no communication).

Entry point for training a QMIX agent whose per-agent network matches the
NDQ RNN architecture (obs + agent_id + prev_action -> GRU -> Q-head) but
without the NDQ communication additions. Implementation lives entirely in
``utils/marl_rnn_qmix.py`` so the feature can be removed by deleting this
file and that one.
"""
...
from utils.marl_rnn_qmix import (
    MARLRNNQMIXLearner,
    RNNAgent,
    RNNQMIXEpisodeCollector,
    RNNQMIXEpisodeReplayBuffer,
    load_rnn_qmix_training_state,
    make_rnn_eval_action_selector,
    rnn_collect_transition,
    save_rnn_qmix_checkpoint,
    save_rnn_qmix_training_state,
)
...
def parse_args() -> argparse.Namespace:
    parser = build_base_qlearning_parser(
        description="Train recurrent QMIX (NDQ-style GRU agent, no communication).",
    ...
    parser.add_argument("--buffer-size", type=int, default=5000,
                        help="Episode replay buffer capacity (matches NDQ default).")
...
def _recurrent_observation_override() -> Dict[str, Any]:
    """Force the NDQ-style recurrent observation profile.

    The GRU hidden state carries history, so history-window observation
    features are disabled to avoid double-counting history.
    """
    return {
        "history_window": 0,
...
def _evaluate_and_log(
    *,
    ...
    action_selector, reset_eval = make_rnn_eval_action_selector(
        agent, n_eval_envs, n_agents, n_actions, device, rnn_layers, rnn_hidden_dim,
    )
    ...
    print(
        f"[RNN-QMIX] Eval at step {global_step}: "
...
def main() -> None:
    ...
    # channel (matches algorithms/marl_qmix.py). The obs/state arrays are reused
    ...
    replay_buffer = RNNQMIXEpisodeReplayBuffer(
    ...
    if resuming:
        ...
        counters = load_rnn_qmix_training_state(
    ...
    with rewards_csv_path.open(csv_mode, newline="", encoding="utf-8") as train_f, \
        ...
        collector = RNNQMIXEpisodeCollector(args.n_envs, device)
        ...
        while global_step < args.total_timesteps:
            step_result = rnn_collect_transition(
                env=env,
                agent=learner.agent,
                obs_raw=obs_raw,
            ...
            # Drain completed episodes from the collector into the replay
            # buffer. One training update per new-episodes batch matches the
            # NDQ cadence (not per env step).
