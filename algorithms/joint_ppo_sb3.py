...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a centralized joint-action PPO (Stable-Baselines3) on the multi-agent NCS env. "
...
class JointTrainEvalCallback(BaseCallback):
    ...
    def _run_eval(self, step: int) -> None:
        ...
        print(
            f"[JOINT_PPO_SB3] Eval at step {step}: "
            ...
            if np.any(dones_arr):
                ...
                for env_idx in np.where(dones_arr)[0]:
                    ...
                    if self.episode % self.log_interval == 0:
                        print(
                            f"[JOINT_PPO_SB3] episode={self.episode} steps={step} "
...
def main() -> None:
    if PPO is None:
        raise ImportError(
            "stable-baselines3 is required for algorithms.joint_ppo_sb3. "
    ...
    save_config_with_hyperparameters(run_dir, args.config, "joint_ppo_sb3", hyperparams)
