...
def _apply_arch_args(args: argparse.Namespace, arch: Dict[str, Any]) -> None:
    ...
    args.feature_norm = arch.get("feature_norm", False)
    args.layer_norm = arch.get("layer_norm", False)
...
def main() -> None:
    ...
    with rewards_csv_path.open(csv_mode, newline="", encoding="utf-8") as train_f, \
        ...
        while global_step < args.total_timesteps:
            ...
            # Normalize advantages globally (across all agents/time/envs) before
            # the sequential loop so that the importance-weighting factor chain
            # sees a consistent scale.  HARL reference: on_policy_ha_runner.py.
